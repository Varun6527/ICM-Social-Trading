...
// src/app/shared/component/languagetranslation/base.language.translation.component.ts
var BaseLanguageTranslationComponent = class {
  translate_base;
  authService;
  constantVaribale;
  languages = [];
  selectedLangObj = {};
  userConfig;
  constructor(translate_base, authService) {
    this.translate_base = translate_base;
  ...
  setDefaultLanguage() {
    ...
    this.languages = this.languages.filter((o) => this.userConfig.allowedLangs.includes(o.code));
    if (this.languages.length <= 0)
      return;
    let savedLangCode = localStorage.getItem("language") || "en";
    savedLangCode = this.languages.some((o) => o.code == savedLangCode) ? savedLangCode : this.languages[0]?.code;
    this.selectedLangObj = this.languages.filter((o) => o.code == savedLangCode)[0] || this.languages[0];
    this.translate_base.setDefaultLang(this.selectedLangObj.name);
    this.translate_base.use(this.selectedLangObj.name);
  }
  selectLanguage(langObj) {
    this.selectedLangObj = langObj;
    this.selectedLangObj = this.languages.filter((o) => o.code == langObj.code)[0] || this.languages[0];
    this.translate_base.use(this.selectedLangObj.name);
...
//# sourceMappingURL=chunk-IN7YTLUY.js.map
